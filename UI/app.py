...
with gr.Blocks(title="企业财务报销助手") as demo:
    ...
    login_btn.click(
        ...
        outputs=[user_state, login_area, main_area, user_info_bar, tabs_container, tab_js_injector, chatbot_display, user_storage, login_msg, history_display]
    ).then(
        fn=send_greeting,
    ...
    logout_btn.click(
        fn=do_logout,
        inputs=user_state,
        outputs=[user_state, login_area, main_area, user_info_bar, tabs_container, tab_js_injector, chatbot_display, user_storage, history_display],
        js="""
        () => {
            try {
